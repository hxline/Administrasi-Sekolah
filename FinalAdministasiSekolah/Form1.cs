...
namespace FinalAdministasiSekolah
{
    public partial class Form1 : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            radCari.Visible = false;
            textCari.Visible = false;
        }
        ...
        private void radButtonTU_Click(object sender, EventArgs e)
        {
            ...
            radButtonUbah.Visible = false;
            radButtonHapus.Visible = false;
            radCari.Visible = true;
            textCari.Visible = true;
        ...
        private void radButtonLaporan_Click(object sender, EventArgs e)
        {
            tombol = "laporan";
            radGridPembayaran.Visible = false;
            radGridSiswa.Visible = false;
            ...
            radGridTabunganSaldo.Visible = false;
            radGridTabunganKeluar.Visible = false;
            radLabelMulai.Text = "Administrasi Sekolah [Laporan]";
            radButtonTotal.Visible = false;
            radButtonTambah.Visible = false;
            radButtonUbah.Visible = false;
            radButtonHapus.Visible = false;
        ...
        private void radButtonElementLaporan_Click(object sender, EventArgs e)
        {
            tombol = "laporan";
            radGridPembayaran.Visible = false;
            radGridSiswa.Visible = false;
            ...
            string path = System.IO.Path.Combine(Environment.CurrentDirectory,
                                     @"..\..\..\Laporan_Final\Laporan_Final\bin\Debug\Laporan_final.exe");
            ProcessStartInfo start = new ProcessStartInfo();
            start.FileName = path; 
//@"D:\Matkul\Semester 5\SBD\FinalAdministasiSekolah\Laporan_Final\Laporan_Final\bin\Debug\Laporan_final.exe";
        ...
        private void BindDataTU()
        {
            ...
            using (koneksi.koneksiDB)
            {
                ...
                string select = "select id_tu as 'ID Tata Usaha', nama as Nama, date_format(tanggal,'%d-%m-%Y') as 'Tanggal Daftar' from ttu";
        ...
        private void BindDataSiswa()
        {
            ...
            using (koneksi.koneksiDB)
            {
                ...
                string select = "select nis as NIS, nama as Nama, kelas as Kelas, date_format(tanggal,'%d-%m-%Y') as 'Tanggal Masuk' from tsiswa";
        ...
        private void BindDataPembayaran()
        {
            ...
            using (koneksi.koneksiDB)
            {
                ...
                string select = "select pem.id_pembayaran as ID, pem.nis as NIS, sis.nama as Nama, sis.kelas as Kelas, " +
                                ...
                                "pem.total as Total, date_format(pem.tanggal,'%d-%m-%Y') as 'Tanggal Bayar', tu.nama as 'Nama TU' " +
        ...
        private void BindDataSurat()
        {
            ...
            using (koneksi.koneksiDB)
            {
                ...
                string select = "select surat.id_surat as ID, tu.nama as 'Nama TU', surat.no_surat as 'No Surat', surat.lampiran as Lampiran, " +
                                "surat.perihal as Perihal, surat.jenis as Jenis, date_format(surat.tanggal,'%d-%m-%Y') as Tanggal,  " +
        ...
        private void BindDataDaftar()
        {
            ...
            using (koneksi.koneksiDB)
            {
                ...
                string select = "select daf.id_daftar as ID, daf.nis as NIS, sis.nama as Nama, sis.kelas as Kelas, " +
                                "daf.jenis as Jenis, daf.jumlah as Jumlah, date_format(daf.tanggal,'%d-%m-%Y') as 'Tanggal Bayar', tu.nama as 'Nama TU' " +
                                "from tdaftar as daf " +
        ...
        private void radGridPendaftaran_CellDoubleClick(object sender, GridViewCellEventArgs e)
        {
            ...
            {
                MessageBox.Show("Sudah kelas " + kelas, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void BindDataTabungan()
        {
            ...
            using (koneksi.koneksiDB)
            {
                ...
                string select = "select tab.id_tabung as ID, tab.nis as NIS, sis.nama as Nama, sis.kelas as Kelas, " +
                                "tab.jumlah as Jumlah, date_format(tab.tanggal,'%d-%m-%Y') as Tanggal, tu.nama as 'Nama TU' " +
        ...
        private void radGridTabunganKeluar_CellDoubleClick(object sender, GridViewCellEventArgs e)
        {

        }

        private void radGridTabunganKeluar_SelectionChanged(object sender, EventArgs e)
        {
            foreach (GridViewRowInfo row in radGridTabunganKeluar.SelectedRows)
            {
                idtab = row.Cells[0].Value.ToString();
                id = row.Cells[1].Value.ToString();
                nama = row.Cells[2].Value.ToString();
        ...
        private void radGridTabunganSaldo_SelectionChanged(object sender, EventArgs e)
        {
            foreach (GridViewRowInfo row in radGridTabunganSaldo.SelectedRows)
            {  
                id = row.Cells[0].Value.ToString();
                saldo = row.Cells[3].Value.ToString();
        ...
        private void radButtonHapus_Click(object sender, EventArgs e)
        {
            ...
            else if (tombol == "tabungan")
            {
                ...
                using (koneksi.koneksiDB)
                {
                    ...
                    string update = "update ttotal set total = total - @jumlah where nis = @id";

                    MySqlCommand cmd = new MySqlCommand(delete, koneksi.koneksiDB);
                    MySqlCommand cmdi = new MySqlCommand(update, koneksi.koneksiDB);

                    cmd.Parameters.AddWithValue("@id", idtab);
                    cmdi.Parameters.AddWithValue("@id", idtab);
            ...
            else if (tombol == "tabungankeluar")
            {
                ...
                using (koneksi.koneksiDB)
                {
                    ...
                    string delete = "delete from ttabungankeluar where id_tabung_keluar = @id";
        ...
        private void radCari_Click(object sender, EventArgs e)
        {
            ...
            else if (tombol == "tabungankeluar")
            {
                DataView cari = new DataView(table);
                cari.RowFilter = string.Format("nama like '%{0}%'", textCari.Text);
                radGridTabunganKeluar.DataSource = cari;
            }
        }
        ...
        private void radButtonTotal_Click(object sender, EventArgs e)
        {
            if (tombol == "total")
            {
                AmbilTabungan ambil = new AmbilTabungan();
                ambil.idTU = idTU;
                ambil.nis = id;
                ambil.saldo = saldo;
                ambil.ShowDialog();
        ...
        private void radButtonPilihLaporan_Click(object sender, EventArgs e)
        {
            Process.Start(@"D:\Matkul\Semester 5\SBD\FinalAdministasiSekolah\Laporan_Final\Laporan_Final\bin\Debug\Laporan_final.exe");
            
        }

        //
        //// Akhir Tombol Pilih Laporan
        //
        ...
        private void ribonUtama_Click(object sender, EventArgs e)
        {

        }

        //
        ////Akhir Tombol Ganti Password
        //
